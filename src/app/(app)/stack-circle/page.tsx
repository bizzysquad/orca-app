...
export default function StackCirclePage() {
  ...
  // Theme-synced color scheme
  const teal = theme.accent;
  const tealLight = isDark ? `${theme.accent}26` : `${theme.accent}14`;
  const tealBorder = isDark ? `${theme.accent}55` : `${theme.accent}44`;
            ...
            {groups.length === 0 ? (
              ...
              <>
                ...
                {groups.map((group) => {
                  ...
                  return (
                    <motion.div key={group.id} variants={itemVariants} className="rounded-2xl border overflow-hidden transition-colors" style={{ backgroundColor: theme.card, borderColor: isDark ? `${theme.accent}55` : `${theme.accent}44` }}>
                      {/* Group Header Row */}
                      <div className="flex items-center gap-3 p-4 sm:p-5">
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <motion.button
                            ...
                            style={{ backgroundColor: isDark ? `${theme.accent}26` : '#FEF3C7', color: '#D97706' }}
                          >
                      ...
                      <AnimatePresence>
                        {isExpanded && (
                          ...
                          >
                            <div className="px-4 sm:px-5 pb-4 sm:pb-5 space-y-4" style={{ borderTop: `1px solid ${tealBorder}` }}>

                              {/* Compact Group Overview */}
                              <div className="rounded-xl p-3 sm:p-4 mt-4" style={{ backgroundColor: isDark ? `${theme.accent}26` : `${theme.accent}14`, border: `1px solid ${theme.accent}44` }}>
                                {/* Editable Name Row */}
                                <div className="flex items-center justify-between mb-3">
                                  {!isEditingName ? (
                                    <div className="flex items-center gap-2 min-w-0">
                                      <span className="font-bold text-sm truncate" style={{ color: theme.accent }}>{displayGroupName}</span>
                                      <button
                                        onClick={() => {
                                    ...
                                    <div className="flex items-center gap-2 flex-1 mr-2">
                                      <input
                                        ...
                                        style={{ backgroundColor: theme.card, color: theme.text, border: `1px solid ${theme.accent}` }}
                                        autoFocus
                                      />
                                      <button onClick={() => handleSaveGroupName(group.id)} className="px-2 py-1 rounded-lg text-xs font-bold" style={{ backgroundColor: theme.accent, color: '#fff' }}>Save</button>
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  <div className="relative flex-shrink-0" style={{ width: 64, height: 64 }}>
                                    <svg width="64" height="64" viewBox="0 0 64 64">
                                      <circle cx="32" cy="32" r="26" fill="none" stroke={`${theme.accent}40`} strokeWidth="6" />
                                      <motion.circle
                                        cx="32" cy="32" r="26" fill="none" stroke={theme.accent} strokeWidth="6"
                                        strokeLinecap="round"
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                                      <div style={{ fontSize: 11, fontWeight: 800, color: theme.accent, lineHeight: 1 }}>{pct(group.current, group.target)}%</div>
                                    </div>
                                  </div>
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-baseline gap-1 mb-1">
                                      <span style={{ fontSize: 18, fontWeight: 900, color: theme.accent }}>{fmt(group.current)}</span>
                                      <span className="text-xs" style={{ color: theme.accent, opacity: 0.7 }}>saved</span>
                                    </div>
                                    <div className="text-xs mb-2" style={{ color: theme.textM }}>
                                      Target: <span style={{ fontWeight: 700, color: theme.accent }}>{fmt(group.target)}</span>
                                    </div>
                                    <div className="w-full rounded-full overflow-hidden" style={{ height: 4, backgroundColor: `${theme.accent}40` }}>
                                      <motion.div
                                    ...
                                    {group.date && (
                                      <div className="flex items-center gap-1 mt-1.5 text-xs" style={{ color: theme.accent }}>
                                        <Calendar size={10} />
                                        {new Date(group.date + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                        <span className="px-1.5 py-0.5 rounded-full" style={{ background: theme.accent, color: '#fff', fontWeight: 700, fontSize: 9 }}>
                                          {(() => {
                                            const now = new Date();
                              ...
                              <div className="flex gap-1.5 rounded-xl p-1" style={{ backgroundColor: isDark ? `${theme.accent}26` : `${theme.accent}14`, border: `1px solid ${theme.accent}44` }}>
                                {[
                                  ...
                                  <button
                                    ...
                                    style={{
                                      backgroundColor: subTab === key ? theme.accent : 'transparent',
                                      color: subTab === key ? '#fff' : theme.accent,
                                    }}
                                  >
                                    {label}
                                  </button>
                                  ...
                                  <div>
                                    ...
                                    <div className="space-y-2">
                                      {group.members.map((member) => (
                                        ...
                                        >
                                          <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs flex-shrink-0" style={{ backgroundColor: theme.accent, color: '#fff', fontWeight: 700 }}>
                                            {member.name[0]}
                                          </div>
                                          <div className="flex-1 min-w-0">
                                          ...
                                          <div className="text-right flex-shrink-0">
                                            <p style={{ fontWeight: 800, color: theme.accent, fontSize: 14 }}>{fmt(member.balance)}</p>
                                            <span className="inline-block text-xs px-2 py-0.5 rounded-full mt-0.5" style={{ backgroundColor: isDark ? `${theme.accent}26` : `${theme.accent}14`, color: theme.accent, fontWeight: 600 }}>
                              ...
                              {subTab === 'trip' && (
                                <div className="space-y-4">
                                  ...
                                  {group.trip && (group.trip.location || group.trip.startDate) && (
                                    <div className="rounded-xl p-3 text-sm" style={{ backgroundColor: isDark ? `${theme.accent}26` : `${theme.accent}14`, border: `1px solid ${theme.accent}44` }}>
                                      <p className="font-bold text-xs uppercase tracking-wider mb-2" style={{ color: theme.accent, opacity: 0.7 }}>Current Trip</p>
                                      {group.trip.location && <p className="font-semibold" style={{ color: theme.accent }}>📍 {group.trip.location}</p>}
                                      {group.trip.startDate && <p className="text-xs mt-1" style={{ color: theme.accent }}>
