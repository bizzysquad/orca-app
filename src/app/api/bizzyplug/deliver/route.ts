...
export async function POST(req: NextRequest) {
  try {
    ...
    const { to, clientName, projectType, message, downloadUrl, subject } = await req.json()
    ...
    const html = `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#F8FAFC;color:#1E293B;padding:32px;border-radius:16px;border:1px solid #E2E8F0;">
        ...
        ${message ? `<p style="font-size:15px;line-height:1.7;color:#334155;margin:0 0 24px;">${message.replace(/\n/g, '<br>')}</p>` : ''}
        ${downloadUrl ? `
        <div style="margin:0 0 32px;text-align:center;">
          <a href="${downloadUrl}" style="display:inline-block;padding:14px 36px;background:#9333EA;color:#fff;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px;">Download Your Files</a>
        </div>` : ''}
        <div style="margin:0 0 24px;padding:20px;background:#F3E8FF;border-radius:12px;border:1px solid #C084FC33;text-align:center;">
          ...
          <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://orcafin.app'}/bizzyplug#leave-review" style="display:inline-block;padding:10px 28px;background:#9333EA;color:#fff;border-radius:8px;text-decoration:none;font-weight:700;font-size:13px;">Leave a Review</a>
        </div>
    ...
    await t.sendMail({
      ...
      to,
      subject: subject ? `${subject} — BizzyPlug` : `Your ${service} is ready! — BizzyPlug`,
      html,
