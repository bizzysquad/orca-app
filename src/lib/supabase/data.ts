...
export async function saveUserData(data: OrcaData): Promise<void> {
  ...
  try {
    ...
    const profileUpdate = {
      ...
      credit_score_transunion: data.user.creditScoreTransUnion || 0,
      credit_score_equifax: data.user.creditScoreEquifax || 0,
      credit_score_experian: data.user.creditScoreExperian || 0,
